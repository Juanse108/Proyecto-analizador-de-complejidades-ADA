...
export class RecursionTreeService {
   ...
   */
  analyzeComplexity(response: AnalyzeResponse): { type: 'recursive' | 'iterative' | 'unknown', tree?: RecursionTree, table?: TraceTable } {
    const bigO = response.big_o.toLowerCase();
    const notes = response.notes || [];

    // Detectar si es recursivo o iterativo
    const isRecursive = notes.some(n => 
      n.toLowerCase().includes('recursiv') || 
      n.toLowerCase().includes('divide') ||
      n.toLowerCase().includes('conquer')
    );

    const isIterative = notes.some(n => 
      n.toLowerCase().includes('iterativ') ||
      n.toLowerCase().includes('loop') ||
      n.toLowerCase().includes('for') ||
      n.toLowerCase().includes('while')
    );
  ...
  private generateRecursionTree(bigO: string, response: AnalyzeResponse): RecursionTree {
    ...
    return {
      ...
      height,
      totalCost,
      description: `Árbol de recursión para ${response.normalized_code.split('\n')[0]}`
    };
  }
   ...
   */
  private buildRecursiveNode(maxLevel: number, currentLevel: number, width: number): RecursionNode {
    ...
    if (currentLevel < maxLevel - 1) {
      ...
      children.push(this.buildRecursiveNode(maxLevel, currentLevel + 1, childWidth));
      children.push(this.buildRecursiveNode(maxLevel, currentLevel + 1, childWidth));
    }
  ...
  private generateTraceTable(bigO: string, response: AnalyzeResponse): TraceTable {
    ...
    if (bigO.includes('n²') || bigO.includes('n^2')) {
      ...
      cumulativeCost = 'log n';
    }

    return {
      iterations,
      totalCost: `O(${cumulativeCost})`
    };
  }
}